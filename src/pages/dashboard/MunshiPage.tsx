...
import { fetchMunshiSummary, adjustMunshi } from '@/lib/pos-api';
import { MunshiSummary } from '@/lib/pos-types';
import { UserCog, RefreshCcw, ArrowDownCircle, ArrowUpCircle, ChevronDown, ChevronUp, Download } from 'lucide-react';
import { useToast } from '@/lib/toast';

// Same All/Today/This Month/Custom history-range pattern as BankPage.tsx/
// GrainStockPage.tsx's own local copy.
type MunshiHistoryRangeMode = 'all' | 'today' | 'month' | 'custom';
...
// The shop's own Munshi Khata - a single running balance (unlike Bank/
// Grain Stock, which can have several named accounts - see
// backend/models/MunshiAccount.js's own comment on why). Fed automatically
// whenever a Customer Dues "+ Add Dues"/"- Pay Dues" action is made "via
// Munshi" (see DuesPage.tsx's payment-method picker) - that ALSO still
// moves Cash in Hand, since Munshi is real cash the shop is tracking
// separately, not a replacement pool of money the way Bank/Grain Stock
// are. Manual add/withdraw here (e.g. actually paying the munshi their
// commission) never touches any customer.
export default function MunshiPage() {
  const { toast, confirm } = useToast();
  const [summary, setSummary] = useState<MunshiSummary | null>(null);
  ...
  const [rangeMode, setRangeMode] = useState<MunshiHistoryRangeMode>('all');
  ...
  const loadSummary = async () => {
    setLoading(true);
    try {
      const data = await fetchMunshiSummary();
      if (data) setSummary(data);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Could not load Munshi Khata.');
    } finally {
      setLoading(false);
    }
  };
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      await downloadPdfDocument(doc, `munshi_khata_statement.pdf`);
    } finally {
  ...
  const record = async (direction: 'in' | 'out') => {
    ...
    if (direction === 'out') {
      const confirmed = await confirm(`Withdraw ₨${amountValue} from Munshi Khata?`, { title: 'Withdraw', confirmText: 'Withdraw', tone: 'danger' });
      if (!confirmed) return;
    }
    setSaving(true);
    try {
      const updated = await adjustMunshi(amountValue, direction, note.trim());
      if (!updated) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            Munshi Khata <UserCog className="text-cyan-600" size={32} />
          </h1>
          <p className="text-slate-500 font-bold">
            Money tracked for the munshi - a Customer Dues payment made "via Munshi" moves both this khata and Cash in Hand together.
          </p>
        </div>
        <button
          type="button"
      ...
      {loading ? (
        <div className="text-center p-10 text-slate-500 font-bold">Loading Munshi Khata...</div>
      ) : (
        <div className="bg-white rounded-[28px] border border-slate-200 shadow-sm p-6 space-y-5">
          <div className="flex items-start justify-between gap-2">
            ...
            <button
              ...
              title="Download the complete Munshi Khata statement as a PDF"
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input
                ...
                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-cyan-500"
              />
            </div>
            ...
            {showHistory ? (
              <div className="mt-2 space-y-2">
                <div className="flex flex-wrap gap-1.5">
                  {(['all', 'today', 'month', 'custom'] as MunshiHistoryRangeMode[]).map((mode) => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => setRangeMode(mode)}
                      className={`px-2.5 py-1 rounded-lg text-[10px] font-black transition-colors ${
                        rangeMode === mode ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                      }`}
                    >
                ...
                {rangeMode === 'custom' ? (
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="flex-1 p-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-bold outline-none focus:border-cyan-500"
                    />
