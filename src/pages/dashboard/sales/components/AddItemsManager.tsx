...
type OrderItemDraft = { name: string; price: number; quantity: number; variation: string };
        ...
        {tab === 'items' ? (
          <>
            <div className="relative">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products" className="w-full rounded-full border border-gray-200 bg-[#F8F9FB] py-4 pl-12 pr-4 outline-none" />
            </div>
            ...
            <div className="grid max-h-[360px] gap-3 overflow-y-auto pr-1 sm:grid-cols-2">
              {visibleProducts.map((product) => (
                <button key={product.id} type="button" onClick={() => addDraft({ name: product.name, price: product.price, quantity: 1, variation: product.variation })} className="group flex items-center gap-3 rounded-[24px] border border-transparent bg-[#FAFBFC] p-3 text-left transition hover:border-[#D6E332]">
                  <div className="relative h-20 w-20 overflow-hidden rounded-2xl bg-indigo-50 p-2">
                    <img src={getProductImageUrl(product.image)} alt={product.name} className="w-full h-full object-contain drop-shadow-sm transition duration-300 group-hover:scale-105" />
                  </div>
                  <div>
            ...
            <div className="rounded-[24px] bg-[#F8F9FB] p-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <input value={customItem.name} onChange={(event) => setCustomItem((previous) => ({ ...previous, name: event.target.value }))} placeholder="Item name" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
                <input value={customItem.variation} onChange={(event) => setCustomItem((previous) => ({ ...previous, variation: event.target.value }))} placeholder="Variation / size" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
                <input value={customItem.price} onChange={(event) => /^\d*$/.test(event.target.value) && setCustomItem((previous) => ({ ...previous, price: event.target.value }))} placeholder="Price" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
                <input value={customItem.quantity} onChange={(event) => /^\d*$/.test(event.target.value) && setCustomItem((previous) => ({ ...previous, quantity: event.target.value }))} placeholder="Quantity" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              </div>
              <button
                type="button"
                onClick={() => {
          ...
          <div className="space-y-4">
            <div className="grid gap-2 sm:grid-cols-2">
              <input value={productForm.name} onChange={(event) => setProductForm((previous) => ({ ...previous, name: event.target.value }))} placeholder="Product name" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              <input value={productForm.variation} onChange={(event) => setProductForm((previous) => ({ ...previous, variation: event.target.value }))} placeholder="Variation" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              <input value={productForm.price || ''} onChange={(event) => /^\d*$/.test(event.target.value) && setProductForm((previous) => ({ ...previous, price: Number(event.target.value || 0) }))} placeholder="Price" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              <input value={productForm.stock || ''} onChange={(event) => /^\d*$/.test(event.target.value) && setProductForm((previous) => ({ ...previous, stock: Number(event.target.value || 0) }))} placeholder="Stock" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              <input value={productForm.category} onChange={(event) => setProductForm((previous) => ({ ...previous, category: event.target.value }))} placeholder="Category" className="rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
              <select value={productForm.image} onChange={(event) => setProductForm((previous) => ({ ...previous, image: event.target.value }))} className="rounded-2xl border border-gray-200 px-4 py-3 outline-none">
                {imageOptions.map((option) => <option key={option} value={option}>{option.split('/').pop()}</option>)}
              </select>
            </div>
            <textarea value={productForm.description} onChange={(event) => setProductForm((previous) => ({ ...previous, description: event.target.value }))} placeholder="Description" className="min-h-24 w-full rounded-2xl border border-gray-200 px-4 py-3 outline-none" />
            ...
            <div className="space-y-2">
              {products.map((product) => (
                <div key={product.id} className="flex items-center justify-between rounded-[20px] bg-[#F8F9FB] px-4 py-3">
                  <div>
