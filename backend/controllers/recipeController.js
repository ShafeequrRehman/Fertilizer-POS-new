...
// round-trip per variation. Also relied on by services/stockService.js's
// automatic ingredient-stock deduction on every order (queried directly
// there, not via this API), and by the offline sync snapshot
// (offline-sync.ts's pushIngredientsCache, consumed by
// IngredientStockSection.tsx's offline consumption estimate).
...
// Defines/replaces the WHOLE recipe for exactly one Product document in a
// single call (upsert), same "replace the full list, don't diff it"
// approach applyOrderPatch already uses for an order's replaceItems action.
// A product that's simply resold AS its own raw stock (e.g. a fertilizer
// bag sold under the same name it's tracked under in Ingredient Stock)
// just needs one line here - ingredientId of that same-named ingredient,
// quantity 1, its own base unit - so selling 1 deducts exactly 1.
//
